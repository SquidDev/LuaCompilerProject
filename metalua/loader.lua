...
M.metalua_extension_prefixs = {'', 'metalua.extension.'}
...
function M.metalua_loader (name)
	local file, filename_or_msg = M.findfile (name, (M.path..";"):gsub("%.(lua[:;])", ".m%1") :sub(1, -2))
...
function extension (name, mlp)
	local paths = {}
	local _, extend_func
	for _, prefix in ipairs(M.metalua_extension_prefixs) do
		local complete = prefix .. name
		_, extend_func = pcall(require, complete)

		if _ then
			break
		else
			table.insert(paths, complete)
			extend_func = nil
		end
	end

	if not extend_func then
		error("Cannot find " .. name .. " at ".. table.concat(paths, ", "))
	end

	if not mlp.extensions[name] then
		mlp.extensions[name] = extend_func(mlp)
	end
end

...
if not filename then
	print("Usage")
	print("lua profile.lua [-v] <profile_log>")
...
if not file then error("Cannot open " .. filename, 0) end

profiler.start("luaprof.out")
file(unpack(args))
profiler.stop()

file = io.open("luaprof.out")
if not file then error("Cannot open result", 0) end

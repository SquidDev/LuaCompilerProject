...
type Literal(lang : Language) = 
    ...
    let stringVal = 
        let escapeNumber = manyMinMaxSatisfy 1 3 Char.IsDigit |>> (fun x -> Byte.Parse x |> Convert.ToChar)
        ...
        let escape = pchar '\\' >>. (escapeSequence <|> escapeNumber) <?> "escape sequence"
        let string x = betweenL (pchar x) (pchar x) (manyChars (escape <|> noneOf [ x ])) "string"
    ...
    let table = 
        ...
        let pair = pipe2 (betweenL (Symbol "[") (Symbol "]") lang.Expression "table key") tableAssign makePair
        ...
        let tableItem = pair <|> identifier <|> (lang.Expression |>> fun x -> new TableNode.TableItem(x))
        let factory x : IValueNode = upcast new TableNode(x)
        betweenL (Symbol "{") (Symbol "}") (sepEndBy tableItem (Symbol "," <|> Symbol ";") |>> factory) "table"

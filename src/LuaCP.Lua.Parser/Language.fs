...
type Language() as this = 
    ...
    let declaration = 
        new NamedReference<IDeclarable, unit>("declaration", 
                                              IdentifierBase 
                                              |>> (fun x -> 
                                              upcast Nodes.Identifier x))
    ...
    let statementLine = statement.AsParser
                        ...
                        .>> (Symbol ";" |> optional)
    // Declaration
    member val DeclarationList = sepBy declaration.AsParser (Symbol ",") 
                                 <?> "declaration list"
    member val DeclarationList1 = sepBy declaration.AsParser (Symbol ",") 
                                  <?> "declaration list"
    ...
    member val ExprList = sepBy expressionNamed (Symbol ",") 
                          <?> "expression list"
    member val ExprList1 = sepBy expressionNamed (Symbol ",") 
                           <?> "expression list"
    ...
    member val ChunkTillEnd = manyTill statementLine (Keyword "end") 
                              |>> Nodes.Block |> withPosition

    ...
    fun (stream : CharStream<_>) -> 
        ...
        if reply1.Status = Ok then 
            ...
            if reply2.Status = Ok then 
                ...
                else Reply(reply3.Status, mergeErrors error3 (notClosedError (state0.GetPosition(stream))))
...
let chain (initial : Parser<'t, 'u>) (remaining : 't -> Parser<'t, 'u>) (input : CharStream<'u>) = 
...
let blacklist (illegal : Set<string>) (p : Parser<string, _>) (stream : CharStream<_>) = 
...
let whitelist (valid : Set<string>) (p : Parser<string, _>) (stream : CharStream<_>) = 
    let current = stream.State
    let result = p stream
...
let inline getPosition (stream : CharStream<_>) = 
    new Position(int32 (stream.Line), int32 (stream.Column), int32 (stream.Index))

let withPosition (p : Parser<#INode, _>) (stream : CharStream<_>) = 
    let start = getPosition stream
    let result = p stream
    if result.Status = Ok then result.Result.Position <- new Range(stream.Name, start, getPosition stream)
    result

...
function CONV:bytecode_to_function(bc, name, env)
	...
	if loadstring then
		return setfenv(loadstring(bc, name), env)
	else
		return load(bc, name, 'bt', env)
	end
end
...
function CONV:ast_to_src(...)
	...
	-- Not written in plain Lua
	return require 'metalua.compiler.source' (...)
end

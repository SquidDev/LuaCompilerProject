...
return function(M)
	...
	local function opf1(op)
		return function (_,a) return { tag="Op", op, a } end
	end

	--- opf2 = |op| |a,_,b| `Op{ op, a, b }
	local function opf2(op)
		return function (a,_,b) return { tag="Op", op, a, b } end
	end

	--- opf2r = |op| |a,_,b| `Op{ op, b, a } -- (args reversed)
	local function opf2r(op)
		return function (a,_,b) return { tag="Op", op, b, a } end
	end
	...
	local function op_ne(a, _, b)
		...
		return {
			tag="Op", "not",
			{ tag="Op", "eq", a, b, lineinfo= { first = a.lineinfo.first, last = b.lineinfo.last } }
		}
	end
	...
	M.expr = gg.expr {
		...
		suffix = {
			name = "expr suffix op",
			{ "[", _M.expr, "]", builder = function (tab, idx) return {tag="Index", tab, idx[1]} end},
			{ ".", _M.id, builder = function (tab, field) return {tag="Index", tab, _M.id2string(field[1])} end },
			{ "(", _M.func_args_content, ")", builder = function(f, args) return {tag="Call", f, unpack(args[1])} end },
			{ "{", _table.content, "}", builder = function (f, arg) return {tag="Call", f, arg[1]} end},
			...
			default = { name="opt_string_arg", parse = _M.opt_string, builder = function(f, arg) return {tag="Call", f, arg } end }
		}
	}
end

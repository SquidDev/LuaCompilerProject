...
local function registerlocalvar (fs, varname)
	fs.f.locvars[fs.nlocvars] = { varname = varname } -- LocVar
...
local function open_func (old_fs)
	...
	return {
		...
		actvar = { },
		f = f,
		prev = old_fs, -- linked list of funcstates
		pc = 0,
		lasttarget = -1,
		jpc = luaK.NO_JUMP,
		freereg = 0,
		...
		nactvar = 0,
		bl = nil,
		nestlevel = old_fs and old_fs.nestlevel or 0,
		lastline = 0,
		forward_gotos = { },
		labels = { },
	}
...
-- This must be done both here for backward gotos, and in
...
function expr.expr (fs, ast, v)
	...
	elseif not ast.tag then
		error ("No tag in expression ".. pp.tostring(ast, {hide_hash=1, metalua_tag=1}))
	else

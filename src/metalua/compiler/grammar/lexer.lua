...
function M.new_position(line, column, offset, source)
	expect(line, "number", "line")
	expect(column, "number", "column")
	expect(offset, "number", "offset")
	expect(source, "string", "source")
...
function M.new_lineinfo(first, last)
	expect(first, 'lexer.position', 'first')
	expect(last, 'lexer.position', 'last')
...
function M.new_comment_line(text, lineinfo, nequals)
	...
	expect(lineinfo, "lexer.lineinfo", "lineinfo")
	expect(nequals, "?number", "nequals")
...
function lexer:extract_number()
	...
	if j then
		j = self.src:match(self.patterns.number_mantissa_hex[1], j) or self.src:match(self.patterns.number_mantissa_hex[2], j)
		if j then
			j = self.src:match(self.patterns.number_exponant_hex, j) or j
		end
	else
		j = self.src:match(self.patterns.number_mantissa[1], self.i) or self.src:match(self.patterns.number_mantissa[2], self.i)
		if j then
...
function lexer:check(...)
	...
	local a = self:next()
	local function err()
		error("Got " .. tostring(a) .. ", expected one of these keywords: '" .. table.concat(words,"', '") .. "'") end

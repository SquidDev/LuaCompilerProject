...
local function raw_parse_sequence(lx, p)
	...
	for i=1, #p do
		...
		elseif M.is_parser(e) then
			...
			error(string.format("Sequence `%s': element #%i is neither a string nor a parser: %s", p.name, i, pp.tostring(e)) )
		end
	end
	return r
end
...
function M.onkeyword(p)
	...
	for _, x in ipairs(p) do
		if type(x)=="string" then
			...
			assert(not p.primary and M.is_parser(x)); p.primary = x
		end
	end
...
--- The future method provides a way of lazy binding values
-- It is recommended even when not required as it enables other extensions
-- to change the parser for a keyword
